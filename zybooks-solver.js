...
(function () {
	...
	const checkForProblems = setInterval(() => {
		...
		if (
			...
			// Complete multiple choice problems & render webpage
			setTimeout(() => {
				// Multiple Choice Questions
				const questions = document.getElementsByClassName("question-choices");

				// Render Webpage Buttons (for HTML courses)
				const renderWebpageButtons =
					document.getElementsByClassName("render-webpage");

				// Click all render webpage buttons
				if (renderWebpageButtons.length > 0) {
					for (const renderWebpageButton of renderWebpageButtons) {
						renderWebpageButton.click();
					}
				}

				// First pass, just check the first option for multiple choice
